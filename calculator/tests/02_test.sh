...
if ! $CALCULATOR 6 - 3; then  # If the return code of $PROGRAM is non-zero (i.e. error)...
  echo 'ERROR! A valid run of the calculator (6 - 3) failed!'
  exit 1
fi

# Test 02: Ensure simple case has correct output...
if [[ ($CALCULATOR 6 - 3) -ne 3 ]]; then  # If the output of the program is not 2...
  echo 'ERROR! A valid run of the calculator (6 - 3) failed to produce 3 as an output!'
  exit 1
fi

# Test 03: Ensure program errors with an invalid operand
if $CALCULATOR 6 @ 3; then  # If the return code of $PROGRAM is zero (i.e. success)...
  echo 'ERROR! An invalid run of the application (6 @ 3) apparently succeeded?!'
  exit 1
fi
